calibration_document = '''nqninenmvnpsz874
...
# Sure, I could make this better and prettier... but why should I? Its just a game.
def insert_number_where_word(row):
  while True:
    idx = row.find('one')
    if idx == -1:
      break
    row = row[:idx+1] + '1' + row[idx+1:]
  while True:
    idx = row.find('two')
    if idx == -1:
      break
    row = row[:idx+1] + '2' + row[idx+1:]
  while True:
    idx = row.find('three')
    if idx == -1:
      break
    row = row[:idx+1] + '3' + row[idx+1:]
  while True:
    idx = row.find('four')
    if idx == -1:
      break
    row = row[:idx+1] + '4' + row[idx+1:]
  while True:
    idx = row.find('five')
    if idx == -1:
      break
    row = row[:idx+1] + '5' + row[idx+1:]
  while True:
    idx = row.find('six')
    if idx == -1:
      break
    row = row[:idx+1] + '6' + row[idx+1:]
  while True:
    idx = row.find('seven')
    if idx == -1:
      break
    row = row[:idx+1] + '7' + row[idx+1:]
  while True:
    idx = row.find('eight')
    if idx == -1:
      break
    row = row[:idx+1] + '8' + row[idx+1:]
  while True:
    idx = row.find('nine')
    if idx == -1:
      break
    row = row[:idx+1] + '9' + row[idx+1:]
  return row


def replace_letters_in_row(row):
  while True:
    num_as_letters = [
      {'word': 'one', 'number': 1, 'f_index': -1},
      {'word': 'two', 'number': 2, 'f_index': -1},
      {'word': 'three', 'number': 3, 'f_index': -1},
      {'word': 'four', 'number': 4, 'f_index': -1},
      {'word': 'five', 'number': 5, 'f_index': -1},
      {'word': 'six', 'number': 6, 'f_index': -1},
      {'word': 'seven', 'number': 7, 'f_index': -1},
      {'word': 'eight', 'number': 8, 'f_index': -1},
      {'word': 'nine', 'number': 9, 'f_index': -1}
    ]
    for item in num_as_letters:
      num_as_letters[item['number']-1]['f_index'] = (row.find(item['word']))
    if all_are_minus(num_as_letters):
      break 
    filtered_array = [item for item in num_as_letters if item['f_index'] >= 0]
    sorted_array = sorted(filtered_array, key=lambda x: x['f_index'])
    row = row.replace(sorted_array[0]['word'], str(sorted_array[0]['number']))
  return row
def all_are_minus(dicta):
  for item in dicta:
    if item['f_index'] != -1:
      return False
  return True
...
sum = 0
cal_dict = calibration_document.splitlines()
for row in cal_dict:
  row = insert_number_where_word(row)
  # row = replace_letters_in_row(row)
  first_num = ''
  last_num = ''
  first_found = False
  for letter in row:
